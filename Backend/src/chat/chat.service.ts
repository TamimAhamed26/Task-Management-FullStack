...
export class ChatService {
  ...
  async createConversation(userIds: number[], type: 'direct' | 'group'): Promise<Conversation> {
    ...
    if (type === 'direct' && userIds.length === 2) {
        const [user1Id, user2Id] = userIds.sort((a, b) => a - b); // Ensure consistent ordering
        const existingConversation = await this.conversationRepository.createQueryBuilder('conversation')
            ...
            .having('COUNT(conversation.id) = 1') // Ensure exactly two participants
            .getOne();
  ...
  async saveMessage(conversationId: number, senderId: number, content: string): Promise<Message> {
    const conversation = await this.conversationRepository.findOne({
        where: { id: conversationId },
        relations: ['participants'] // Load participants to validate sender
    });
    if (!conversation) {
      throw new NotFoundException('Conversation not found.');
    }
    ...
    const message = this.messageRepository.create({
      conversation,
      sender: { id: senderId } as User, // Cast to User for relation (TypeORM will use id)
      content,
    });
    const savedMessage = await this.messageRepository.save(message);

    // Update conversation's updatedAt to bring it to top of user's conversation list
    conversation.updatedAt = new Date();
    await this.conversationRepository.save(conversation);

    // Update lastReadAt for all participants *except* the sender to mark it as unread for them
    await this.participantRepository.createQueryBuilder()
        .update(ConversationParticipant)
        .set({ lastReadAt: null }) // Set to null for unread by others
